...
import { 
  ...
  Trash2, CheckCircle2, User, Flame, Clock, 
  BarChart3, Camera as CameraIcon, Info, X, Loader2, Sparkles, Video, AlertTriangle, ShieldCheck, UserCircle2,
  ArrowUpCircle
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [newSessionData, setNewSessionData] = useState({ muscles: new Set<MuscleGroup>(), exercises: [] as LoggedExercise[], title: '' });
  ...
  const processPose = useCallback(async (results: any) => {
    ...
    ctx.restore();
  }, [coachPhase, lastFeedback, selectedExercise, isAnalyzing, positionScore]);
  ...
  useEffect(() => {
    ...
    const pose = new PoseConstructor({ locateFile: (f: string) => `https://cdn.jsdelivr.net/npm/@mediapipe/pose/${f}` });
  ...
  useEffect(() => {
    if (countdown === 0) { setCoachPhase('workout'); setCountdown(null); generateCoachSpeech("Let's go."); return; }
  ...
  const handleRequestSimulation = async () => {
    ...
    } catch (e) { setSimulation({ isGenerating: false, videoUrl: null, statusMessage: "Generation failed." }); }
  };
  ...
  const saveWorkout = () => {
    const session: WorkoutSession = {
      ...
      date: new Date(),
      title: `${selectedExercise} AI Session`,
      muscles: [], // Inferred
    ...
    <div className="max-w-4xl w-full bg-zinc-900/50 border border-white/10 rounded-[2.5rem] backdrop-blur-xl overflow-hidden shadow-2xl flex flex-col md:flex-row">
      <div className="md:w-1/2 relative bg-black aspect-video md:aspect-auto">
        {simulation.videoUrl ? (
          <div className="w-full h-full relative">
            ...
            <button onClick={closeSimulation} className="absolute top-4 right-4 p-2 bg-black/60 rounded-full"><X className="w-4 h-4" /></button>
          </div>
        ) : (
      ...
      <div className="p-10 flex-1 flex flex-col space-y-8">
        ...
        <div className="pt-8 border-t border-white/5 flex gap-4">
          <button onClick={onBack} className="px-6 py-4 bg-zinc-800 hover:bg-zinc-700 rounded-2xl font-black uppercase italic">Back</button>
          <button onClick={onStart} className="flex-1 py-4 bg-emerald-500 hover:bg-emerald-400 text-black rounded-2xl font-black uppercase italic text-xl shadow-xl transition-all">Enter Scanning Ground</button>
        </div>
      </div>
    </div>
  </div>
...
const CoachPositioningView: React.FC<{ videoRef: any; canvasRef: any; score: number; countdown: number | null; onReady: () => void; onCancel: () => void }> = ({ videoRef, canvasRef, score, countdown, onReady, onCancel }) => (
  <div className="flex-1 relative flex items-center justify-center">
    ...
    <div className="relative z-10 max-w-md w-full bg-zinc-900/90 border border-white/10 p-10 rounded-3xl shadow-2xl text-center space-y-8">
      ...
      {countdown !== null ? (
        ...
        <button disabled={score < 100} onClick={onReady} className={`w-full py-4 rounded-xl font-black uppercase italic text-xl transition-all ${score === 100 ? 'bg-emerald-500 text-black' : 'bg-zinc-800 text-zinc-600'}`}>
          {score === 100 ? 'Calibrate Session' : 'Awaiting Detection...'}
        </button>
      )}
      <button onClick={onCancel} className="text-zinc-500 hover:text-white font-bold text-xs uppercase tracking-widest">Cancel</button>
    </div>
  </div>
);
...
const CoachWorkoutView: React.FC<{ videoRef: any; canvasRef: any; feedback: CoachingFeedback | null; angles: JointAngles | null; reps: number; onStop: () => void; onRequestSimulation: () => void; simulation: SimulationState; closeSimulation: () => void; }> = ({ videoRef, canvasRef, feedback, angles, reps, onStop, onRequestSimulation, simulation, closeSimulation }) => (
  <div className="flex-1 flex flex-col md:flex-row p-6 gap-6">
    ...
    <div className="w-full md:w-96 flex flex-col gap-6">
      ...
      <div className={`flex-1 p-8 rounded-[2rem] border transition-all duration-500 flex flex-col ${feedback?.status === 'critical' ? 'bg-red-950/20 border-red-500/50' : 'bg-zinc-900/40 border-white/5'}`}>
      ...
      <button onClick={onStop} className="w-full py-5 bg-zinc-800 hover:bg-red-500 rounded-2xl font-black uppercase italic tracking-widest transition-all">End & Persist Session</button>
    </div>
...
const SidebarIcon: React.FC<{ icon: any; active: boolean; onClick: () => void }> = ({ icon, active, onClick }) => (
  <button onClick={onClick} className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${active ? 'bg-zinc-900 text-emerald-500' : 'text-zinc-700 hover:text-white'}`}>{React.cloneElement(icon, { className: 'w-6 h-6' })}</button>
);
...
const ExerciseCard: React.FC<{ icon: any; label: string; onClick: () => void }> = ({ icon, label, onClick }) => (
  <button onClick={onClick} className="bg-zinc-900/40 border border-white/5 p-10 rounded-[2.5rem] hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-all text-center space-y-6 group">
    ...
    <ChevronRight className="w-6 h-6 text-zinc-800 mx-auto" />
  </button>
);
