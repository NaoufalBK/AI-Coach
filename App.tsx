...
const App: React.FC = () => {
  ...
  const [sessions, setSessions] = useState<WorkoutSession[]>(() => {
    ...
    return saved
      ? JSON.parse(saved).map((s: any) => ({ ...s, date: new Date(s.date) }))
      : [];
  });
  ...
  const [selectedNutritionDate, setSelectedNutritionDate] = useState<Date>(
    new Date(),
  );

  // Profile Setup State
  const [tempProfile, setTempProfile] = useState<
    Omit<UserProfile, "calorieGoal" | "proteinGoal">
  >({
    age: 25,
    weight: 75,
    height: 180,
  ...
  // Persistence Effects
  useEffect(
    () => localStorage.setItem(WORKOUT_KEY, JSON.stringify(sessions)),
    [sessions],
  );
  useEffect(
    () => localStorage.setItem(NUTRITION_KEY, JSON.stringify(nutritionLogs)),
    [nutritionLogs],
  );
  useEffect(
  ...
  const [selectedHistoryDate, setSelectedHistoryDate] = useState<Date>(
    new Date(),
  );

  // AI Coaching States
  const [selectedAIExercise, setSelectedAIExercise] = useState<ExerciseType>(
    ExerciseType.SQUAT,
  );
    ...
    processPoseRef.current = (results: any) => {
      ...
      if (results.poseLandmarks) {
        ...
        if (isPositioning) {
          ...
          const visibleCount = keyLandmarks.filter(
          ...
          ctx.lineWidth = 10;
          ctx.strokeStyle =
            score === 100 ? "#10b981" : "rgba(239, 68, 68, 0.4)";
        ...
        if (isAIActive) {
          ...
          if (mediapipeGlobal.drawConnectors) {
            mediapipeGlobal.drawConnectors(
              ctx,
              landmarks,
              mediapipeGlobal.POSE_CONNECTIONS,
              { color: "rgba(16, 185, 129, 0.7)", lineWidth: 5 },
            );
            mediapipeGlobal.drawLandmarks(ctx, landmarks, {
              color: "#ffffff",
              radius: 3,
            });
          }
          ...
          const detectedPhase = detectExercisePhase(
            landmarks,
          ...
          if (phaseHistoryRef.current.length >= 4) {
            ...
            recentPhases.forEach(
              (p) => (phaseCount[p] = (phaseCount[p] || 0) + 1),
            );

            // Find most common phase among key positions
            const keyPhases = Object.entries(phaseCount).filter(
              ([phase, _]) => phase === "top" || phase === "bottom",
            );
            ...
            if (stablePhase) {
              ...
              if (
                isValidTransition &&
                currentTime - lastRepTimeRef.current > 800
              ) {
                ...
                if (!isProtocolAborted()) {
                  generateCoachSpeech("Analyzing form...");
                }

                // Then analyze biomechanics and provide feedback (async)
                analyzeBiomechanics(angles, selectedAIExercise)
                  .then((fb) => {
                    const t = Date.now();
    ...
    const setup = async () => {
      try {
        ...
        if (!MP.Pose || !MP.Camera) {
          setCameraError(
            "MediaPipe libraries not loaded. Please refresh the page.",
          );
          return;
        }
    ...
    return () => {
      ...
      if (pose)
        try {
          ...
          console.warn("pose.close failed", e);
        }
      try {
        stopCoachSpeech();
      } catch (e) {
        console.warn("stopCoachSpeech failed", e);
      }
    };
  ...
  const sessionsForDate = useMemo(() => {
    return sessions.filter(
      (s) => s.date.toDateString() === selectedHistoryDate.toDateString(),
    );
  ...
  const saveWorkout = () => {
    const vol = newWorkout.exercises.reduce(
      (acc, ex) =>
  ...
  return (
    <div className="flex h-screen bg-[#0a0e27] text-white overflow-hidden font-sans">
      <nav className="w-20 bg-gradient-to-b from-zinc-950/80 via-blue-950/40 to-zinc-950/80 border-r border-emerald-500/10 flex flex-col items-center py-10 gap-8 z-50 backdrop-blur-xl shadow-2xl">
        <div className="w-12 h-12 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-2xl flex items-center justify-center text-black shadow-lg shadow-emerald-500/50 mb-4 animate-pulse">
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar relative">
          ...
          <div className="p-12 max-w-7xl mx-auto space-y-16 opacity-0 animate-[fadeIn_0.7s_ease-in_forwards]">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div className="space-y-4">
                <h1 className="text-7xl font-black italic uppercase tracking-tighter leading-none bg-gradient-to-r from-white via-emerald-300 to-emerald-500 bg-clip-text text-transparent">
                  THE <span>CORE</span>
                </h1>
                <p className="text-zinc-400 font-mono text-[10px] uppercase tracking-[0.5em]">
                  Biometric Performance Center
                </p>
              </div>
              <button
                onClick={() => setActiveSection("new-workout")}
                className="group flex items-center gap-4 px-12 py-6 bg-gradient-to-r from-emerald-500 to-emerald-600 text-black rounded-[2rem] font-black uppercase italic tracking-widest hover:shadow-2xl hover:shadow-emerald-500/40 hover:scale-105 active:scale-95 transition-all shadow-xl shadow-emerald-500/30"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <SummaryCard
                label="Volume"
                value={`${(sessions.reduce((acc, s) => acc + s.totalVolume, 0) / 1000).toFixed(1)}k`}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              <div className="lg:col-span-8 bg-gradient-to-br from-zinc-900/40 via-zinc-900/20 to-transparent border border-white/10 p-12 rounded-[4rem] backdrop-blur-xl shadow-2xl hover:shadow-emerald-500/10 transition-all">
                <h2 className="text-3xl font-black italic uppercase mb-10 flex items-center gap-4">
                  <History className="w-7 h-7 text-emerald-500" /> Biometric
                  History
                </h2>
                <div className="space-y-6">
                  {sessions.slice(0, 4).map((s) => (
                    <div
                      key={s.id}
                      className="bg-gradient-to-r from-white/5 to-white/0 border border-white/10 p-8 rounded-[3rem] flex items-center justify-between group hover:border-emerald-500/50 hover:bg-gradient-to-r hover:from-emerald-500/10 hover:to-white/5 transition-all cursor-pointer shadow-lg"
                    >
                      <div className="flex items-center gap-8">
                        <div className="w-16 h-16 bg-gradient-to-br from-emerald-500/20 to-emerald-500/5 rounded-3xl flex items-center justify-center group-hover:from-emerald-500/40 group-hover:to-emerald-500/10 transition-all shadow-lg">
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {s.muscles.map((m) => (
                              <span
                                key={m}
                                className="text-[9px] font-black uppercase tracking-widest bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-md border border-emerald-500/30"
                              >
                                {m}
                              </span>
                            ))}
                      ...
                      <div className="text-right">
                        <p className="text-3xl font-black italic bg-gradient-to-r from-emerald-400 to-emerald-600 bg-clip-text text-transparent tabular-nums">
                  ...
                  {sessions.length === 0 && (
                    <div className="py-24 text-center text-zinc-700 border-2 border-dashed border-white/10 rounded-[4rem]">
                      Awaiting initial biometric input stream.
                    </div>
                  )}
                </div>
              </div>
              <div className="lg:col-span-4 bg-gradient-to-br from-emerald-500 via-emerald-600 to-emerald-700 p-12 rounded-[4rem] text-black shadow-2xl shadow-emerald-500/30 flex flex-col justify-between hover:shadow-emerald-500/50 transition-all">
                <div>
                  <div className="w-16 h-16 mb-8 p-4 bg-black/20 rounded-3xl">
                  ...
                  <p className="text-sm font-medium opacity-90 leading-relaxed mb-8">
                    System analysis recommends prioritizing posterior chain
                    stability for elite kinematic output.
                  </p>
                </div>
                <div className="space-y-4">
            ...
            {!userProfile ? (
              <div className="max-w-xl mx-auto bg-zinc-900 border border-white/5 p-12 rounded-[4rem] space-y-10 shadow-2xl">
                <header className="text-center space-y-4">
                  ...
                  <h2 className="text-5xl font-black italic uppercase tracking-tighter">
                    METABOLIC <span className="text-emerald-500">INIT</span>
                  </h2>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <InputGroup
                    ...
                    onChange={(v) =>
                      setTempProfile({ ...tempProfile, weight: Number(v) })
                    }
                  ...
                  <InputGroup
                    ...
                    onChange={(v) =>
                      setTempProfile({ ...tempProfile, height: Number(v) })
                    }
                  ...
                  <InputGroup
                    ...
                    onChange={(v) =>
                      setTempProfile({ ...tempProfile, age: Number(v) })
                    }
                  />
                ...
                <button
                  ...
                  className="w-full py-8 bg-emerald-500 text-black rounded-[3rem] font-black uppercase italic tracking-widest flex items-center justify-center gap-4 hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-emerald-500/20 disabled:opacity-50"
                >
                  {isCalculatingGoals ? (
              ...
              <div className="space-y-16">
                <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                  <div className="space-y-4">
                    <h1 className="text-7xl font-black italic uppercase tracking-tighter leading-none">
                      NUTRITION <span className="text-emerald-500">VAULT</span>
                    </h1>
                    <p className="text-zinc-500 font-mono text-[10px] uppercase tracking-[0.5em]">
                      Neural Macro Integration
                    </p>
                  </div>
                  <button
                    onClick={() => setUserProfile(null)}
                    className="px-8 py-4 bg-zinc-950 border border-white/5 text-[10px] font-black uppercase tracking-widest rounded-full hover:bg-zinc-900 transition-all"
                  >
                    Re-Calibrate Body Profile
                  </button>
                </header>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  ...
                  <MacroProgressCard
                    label="Carbs (g)"
                    current={dayStats.carbs}
                    target={Math.round((userProfile.calorieGoal * 0.4) / 4)}
                    color="text-blue-500"
                  />
                  <MacroProgressCard
                    label="Fats (g)"
                    current={dayStats.fats}
                    target={Math.round((userProfile.calorieGoal * 0.25) / 9)}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                  <div className="lg:col-span-7 bg-zinc-900/30 border border-white/5 p-12 rounded-[4rem] backdrop-blur-xl space-y-12 shadow-2xl">
                    <div className="space-y-4">
                      <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-500">
                        NEURAL FOOD ANALYSIS
                      </h3>
                      <p className="text-zinc-400 text-sm leading-relaxed">
                        Enter your meal description in natural language. Gemini
                        3 will calculate the precise macros and log them into
                        your biometric stream.
                      </p>
                    </div>
                      ...
                      <div className="flex flex-wrap gap-4">
                        {(
                          ...
                          <button
                            ...
                            className={`px-8 py-4 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-3 border transition-all ${selectedMealType === type ? "bg-emerald-500 border-emerald-400 text-black shadow-xl shadow-emerald-500/10 scale-105" : "bg-zinc-950 border-white/5 text-zinc-500 hover:border-white/20"}`}
                          >
                            {type === "breakfast" && (
                      ...
                      {getDayLogs(selectedNutritionDate).length > 0 ? (
                        <div className="space-y-6">
                          {(
                            ...
                            return (
                              <div
                                key={type}
                                className="bg-zinc-950/40 p-10 rounded-[3.5rem] border border-white/5 space-y-6 opacity-0 animate-[slideInFromBottom_0.5s_ease-out_forwards]"
                              >
                                <div className="flex items-center gap-4">
                                  ...
                                  <h4 className="text-xl font-black italic uppercase text-zinc-200">
                                    {type}
                                  </h4>
                                </div>
                                  ...
                                  {meals.map((meal) => (
                                    ...
                                    >
                                      <div className="space-y-1">
                                        <p className="text-lg font-black italic uppercase text-zinc-400 leading-none">
                                          {meal.name}
                                        </p>
                                      ...
                                      <div className="flex items-center gap-6">
                                        <p className="text-2xl font-black italic text-emerald-500 tabular-nums">
                                          {meal.calories}
                        ...
                        <div className="py-24 text-center text-zinc-800 border-2 border-dashed border-white/5 rounded-[4rem] flex flex-col items-center">
                          ...
                          <p className="text-xs font-black uppercase tracking-widest opacity-30">
                            Vault is empty for this timeframe.
                          </p>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="lg:col-span-5 space-y-10">
                    <div className="bg-zinc-900/30 border border-white/5 p-12 rounded-[4rem] backdrop-blur-xl shadow-2xl">
                      ...
                      <div className="grid grid-cols-7 gap-4">
                        {Array.from({ length: 31 }).map((_, i) => {
                          ...
                          const isSelected =
                            selectedNutritionDate.getDate() === day;
                          return (
                            <button
                              key={i}
                              ...
                              className={`aspect-square rounded-2xl border flex flex-col items-center justify-center transition-all ${
                                isSelected
                                  ? "bg-white text-black border-white shadow-2xl scale-110 z-10 font-black"
                                  : hasLogs
                    ...
                    <div className="bg-emerald-500 p-12 rounded-[4rem] text-black shadow-2xl shadow-emerald-500/10">
                      <h3 className="text-xl font-black italic uppercase leading-none mb-4">
                        Neural Advice
                      </h3>
                      <p className="text-sm font-medium opacity-80 leading-relaxed">
                        Gemini 3 analysis of your current intake suggests a
                        higher protein bias in the final meal to support muscle
                        protein synthesis based on today's high-volume workout.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="max-w-6xl w-full space-y-16 opacity-0 animate-[fadeIn_1s_ease-in_forwards]">
                  ...
                  <div className="max-w-3xl mx-auto space-y-6">
                    <div className="bg-zinc-900/50 p-12 rounded-[4rem] border border-white/5 space-y-8 shadow-2xl">
                      <p className="text-zinc-300 text-2xl font-medium leading-relaxed italic">
                        Real-time kinematic assessment. Select your discipline
                        below to initiate the tracking protocol.
                      </p>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 pt-10 border-t border-white/5">
                        <StepInstruction
                          num="1"
                          title="Movement"
                          text="Choose your target exercise"
                        />
                        <StepInstruction
                          num="2"
                          title="Calibration"
                        ...
                        <StepInstruction
                          num="3"
                          title="Execution"
                          text="Perform sets with AI feedback"
                        />
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                  {Object.values(ExerciseType)
                    .filter((t) => t !== ExerciseType.CUSTOM)
                    .map((t) => (
                      <button
                        key={t}
                        onClick={() => {
                          setSelectedAIExercise(t);
                          setProtocolAborted(false);
                          setIsPositioning(true);
                        }}
                        className="bg-zinc-900/40 border border-white/5 p-12 rounded-[3.5rem] hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-all group flex flex-col items-center gap-8 shadow-2xl"
                      >
                        <div className="w-20 h-20 bg-white/5 rounded-[2rem] flex items-center justify-center text-zinc-500 group-hover:text-emerald-500 group-hover:scale-110 group-hover:bg-emerald-500/10 transition-all">
              ...
              <div className="relative w-full h-full flex items-center justify-center overflow-hidden bg-black">
                {cameraError ? (
                  <div className="text-center space-y-6 p-12 bg-red-500/10 border border-red-500/40 rounded-[4rem] max-w-2xl">
                    <h3 className="text-3xl font-black text-red-500 uppercase">
                      Camera Error
                    </h3>
                    <p className="text-zinc-300">{cameraError}</p>
                    <button
                      onClick={() => window.location.reload()}
                      className="px-8 py-4 bg-zinc-900 rounded-full font-black uppercase text-xs tracking-widest hover:bg-zinc-800"
                    >
                      Reload Page
                    </button>
                  </div>
                  ...
                  <>
                    ...
                    <div className="absolute top-12 left-12 flex items-center gap-6 bg-black/60 px-8 py-5 rounded-full border border-white/10 backdrop-blur-2xl">
                      ...
                      <p className="text-xs font-black uppercase tracking-widest">
                        Protocol: {selectedAIExercise.replace("_", " ")} •
                        Adjusting stance...
                      </p>
                    </div>
                    ...
                    <div className="absolute bottom-12 right-12 z-20 bg-zinc-950/95 border border-white/10 p-10 rounded-[3.5rem] text-center space-y-8 backdrop-blur-3xl shadow-[0_0_80px_rgba(0,0,0,1)] max-w-xs w-full opacity-0 animate-[slideInFromRight_0.5s_ease-out_forwards]">
                      ...
                      <p className="text-xs text-zinc-500 uppercase tracking-widest leading-relaxed">
                        Move back until your entire silhouette is visible within
                        the green frame.
                      </p>
                      <div className="space-y-4">
                        <div className="h-4 bg-zinc-800 rounded-full overflow-hidden border border-white/5 p-1">
                          <div
                            className={`h-full transition-all duration-700 rounded-full ${positionScore === 100 ? "bg-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.8)]" : "bg-orange-500"}`}
                            style={{ width: `${positionScore}%` }}
                          />
                        </div>
                        <p className="text-[10px] font-black uppercase text-zinc-600 tracking-[0.3em]">
                          {positionScore}% BIO-STABILIZED
                        </p>
                      </div>
                      {countdown !== null && (
                        <div className="text-[10rem] font-black italic text-emerald-500 animate-pulse leading-none">
                          {countdown}
                        </div>
                      )}
                      <button
                        onClick={endSession}
                        className="w-full py-6 bg-zinc-900 hover:bg-red-500/20 text-red-500 rounded-[2rem] font-black uppercase text-[10px] tracking-widest transition-all"
                      >
                        TERMINATE SYNC
                      </button>
                    </div>
              ...
              <div className="w-full h-full flex flex-col md:flex-row gap-12 p-12 opacity-0 animate-[fadeIn_0.5s_ease-in_forwards]">
                  ...
                  <div className="absolute top-12 left-12 flex flex-col gap-4">
                    <div className="px-10 py-5 bg-black/80 border border-white/10 rounded-full flex items-center gap-6 backdrop-blur-2xl">
                      ...
                      <span className="text-xs font-black uppercase tracking-[0.5em]">
                        LIVE KINETICS ANALYZER
                      </span>
                    </div>
                  </div>
                  <div className="absolute bottom-12 right-12 text-right">
                    <p className="text-[16rem] font-black italic text-emerald-500 tracking-tighter leading-none drop-shadow-[0_20px_40px_rgba(0,0,0,0.8)]">
                      {repCount}
                    </p>
                    <p className="text-xs font-black uppercase text-zinc-500 tracking-[1em] mt-4">
                      REPETITIONS
                    </p>
                  </div>
                </div>
                ...
                <div className="w-full md:w-[460px] flex flex-col gap-10">
                  <div className="bg-zinc-900/40 p-12 rounded-[4rem] border border-white/5 space-y-12 backdrop-blur-2xl shadow-2xl">
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-700 flex items-center gap-4">
                      <Activity className="w-6 h-6 text-emerald-500" /> NEURAL
                      DATA STREAM
                    </h3>
                    <div className="space-y-8">
                      <StatMiniRow
                        label="Knee Flexion"
                        value={
                          currentAngles ? `${currentAngles.leftKnee}°` : "--"
                        }
                      />
                      <StatMiniRow
                        label="Hip Kinematics"
                        value={
                          currentAngles ? `${currentAngles.leftHip}°` : "--"
                        }
                      />
                      <StatMiniRow
                        label="Back Integrity"
                        value={
                          currentAngles ? `${currentAngles.backAngle}°` : "--"
                        }
                      />
                    </div>
                  </div>
                  <div
                    className={`flex-1 p-12 rounded-[4rem] border transition-all duration-700 flex flex-col justify-center text-center shadow-2xl ${lastFeedback?.status === "critical" ? "bg-red-500/10 border-red-500/40" : lastFeedback?.status === "warning" ? "bg-orange-500/10 border-orange-500/40" : "bg-zinc-950/80 border-white/5"}`}
                  >
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-700 mb-10 flex items-center justify-center gap-4">
                      <Zap className="w-6 h-6 text-emerald-500" /> AI FEEDBACK
                    </h3>
                    <p className="text-4xl font-black italic text-zinc-100 leading-tight uppercase tracking-tighter">
                      {lastFeedback?.message || "CALIBRATING MOTION..."}
                    </p>
                  </div>
                  <button
                    onClick={endSession}
                    className="w-full py-12 bg-zinc-900 hover:bg-emerald-500 hover:text-black rounded-[3rem] font-black uppercase italic tracking-widest transition-all shadow-3xl active:scale-95"
                  >
                    ABORT PROTOCOL
                  </button>
                </div>
            ...
            <header className="flex items-center justify-between">
              <div>
                ...
                <p className="text-zinc-500 font-mono text-[10px] uppercase tracking-[0.5em] mt-4">
                  Historical Archive of Excellence
                </p>
              </div>
            </header>
              ...
              <div className="lg:col-span-8 bg-zinc-900/30 border border-white/5 p-12 rounded-[4.5rem] backdrop-blur-xl shadow-2xl">
                <div className="grid grid-cols-7 gap-5">
                  {["S", "M", "T", "W", "T", "F", "S"].map((d, idx) => (
                    <div
                      key={`day-${idx}`}
                      className="text-center text-[10px] font-black uppercase tracking-widest text-zinc-800 pb-6"
                    >
                      {d}
                    </div>
                  ))}
                  {Array.from({ length: 31 }).map((_, i) => {
                    const day = i + 1;
                    const date = new Date(2025, 4, day);
                    const hasLogs = sessions.some(
                      (s) => s.date.getDate() === day,
                    );
                    ...
                    return (
                      <button
                        ...
                        className={`aspect-square rounded-[2rem] border flex flex-col items-center justify-center transition-all ${isSelected ? "bg-white text-black border-white shadow-3xl scale-110 z-10" : hasLogs ? "bg-emerald-500/10 border-emerald-500/40 hover:border-emerald-500" : "bg-transparent border-white/5"}`}
                      >
                        <span className="text-3xl font-black italic">
                          {day}
                        </span>
                        {hasLogs && !isSelected && (
                          <div className="w-2 h-2 bg-emerald-500 rounded-full mt-2 shadow-[0_0_10px_rgba(16,185,129,1)]" />
                        )}
                      </button>
                    );
                  })}
              ...
              <div className="lg:col-span-4 space-y-8 max-h-[70vh] overflow-y-auto pr-4 custom-scrollbar">
                {sessionsForDate.length > 0 ? (
                  sessionsForDate.map((s) => (
                    <div
                      key={s.id}
                      className="bg-zinc-900 border border-white/5 p-10 rounded-[4rem] space-y-10 opacity-0 animate-[slideInFromBottom_0.5s_ease-out_forwards] group"
                    >
                      <header className="flex justify-between items-start">
                        <div>
                          <h4 className="text-3xl font-black italic uppercase text-emerald-500 leading-none">
                            {s.title}
                          </h4>
                        ...
                        {s.exercises.map((ex) => (
                          <div key={ex.id} className="space-y-4">
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-lg font-black uppercase italic text-zinc-300">
                                {ex.name}
                              </span>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                              {ex.sets.map((set, idx) => (
                                <div
                                  key={idx}
                                  className="bg-white/5 p-4 rounded-2xl border border-white/5 group-hover:bg-white/10 transition-colors"
                                >
        ...
        {activeSection === "new-workout" && (
          <div className="h-full flex flex-col md:flex-row bg-black opacity-0 animate-[slideInFromBottom_0.7s_ease-out_forwards]">
            <div
              className={`flex-1 p-12 flex flex-col items-center border-r border-white/5 overflow-y-auto custom-scrollbar transition-all duration-500 ${workoutStep === "exercises" ? "md:opacity-40 grayscale-[0.5]" : "opacity-100"}`}
            >
              <div className="w-full max-w-lg space-y-12">
                <header className="text-center space-y-4">
                  <h2 className="text-5xl font-black italic uppercase tracking-tighter">
                    PHASE I: <span className="text-emerald-500">TAGGING</span>
                  </h2>
                  <p className="text-zinc-500 font-mono text-[10px] uppercase tracking-[0.4em]">
                    Identify target muscle streams
                  </p>
                </header>
                <div className="relative aspect-[4/6] w-full bg-zinc-900/30 rounded-[5rem] border border-white/5 p-12 flex flex-col items-center group overflow-hidden shadow-2xl">
                  <div className="relative w-full h-full flex items-center justify-center scale-110">
                    <svg
                      viewBox="0 0 200 450"
                      className="w-full h-full relative z-10 drop-shadow-[0_0_50px_rgba(0,0,0,0.9)]"
                    >
                      <defs>
                        <filter id="logNeon">
                      ...
                      <path
                        d="M100,20 C120,20 135,40 135,65 C135,75 145,80 170,85 L180,180 L160,180 L155,220 L155,420 L115,420 L110,320 L90,320 L85,420 L45,420 L45,220 L40,180 L20,180 L30,85 C55,80 65,75 65,65 C65,40 80,20 100,20 Z"
                      ...
                      {newWorkout.view === "front" ? (
                        <>
                          <DetailedMusclePath
                            name="Chest"
                            active={newWorkout.muscles.has("Chest")}
                            onClick={() => toggleMuscle("Chest")}
                            d="M75,95 Q100,85 125,95 L128,125 Q100,135 72,125 Z"
                          />
                          <DetailedMusclePath
                            name="Shoulders"
                            active={newWorkout.muscles.has("Shoulders")}
                            onClick={() => toggleMuscle("Shoulders")}
                            d="M55,85 Q75,75 85,90 L75,120 Q60,110 55,85 Z M145,85 Q125,75 115,90 L125,120 Q140,110 145,85 Z"
                          />
                          <DetailedMusclePath
                            name="Abs"
                            active={newWorkout.muscles.has("Abs")}
                            onClick={() => toggleMuscle("Abs")}
                            d="M82,135 Q100,128 118,135 L118,195 Q100,205 82,195 Z"
                          />
                          <DetailedMusclePath
                            name="Quads"
                            active={newWorkout.muscles.has("Quads")}
                            onClick={() => toggleMuscle("Quads")}
                            d="M72,215 L95,215 L85,320 L60,320 Z M105,215 L128,215 L140,320 L115,320 Z"
                          />
                          <DetailedMusclePath
                            name="Biceps"
                            active={newWorkout.muscles.has("Biceps")}
                            onClick={() => toggleMuscle("Biceps")}
                            d="M50,110 Q40,140 45,175 L65,175 Q65,140 55,110 Z M150,110 Q160,140 155,175 L135,175 Q135,140 145,110 Z"
                          />
                        </>
                      ) : (
                        <>
                          <DetailedMusclePath
                            name="Back"
                            active={newWorkout.muscles.has("Back")}
                            onClick={() => toggleMuscle("Back")}
                            d="M70,90 Q100,80 130,90 L135,145 Q100,160 65,145 Z"
                          />
                          <DetailedMusclePath
                            name="Triceps"
                            active={newWorkout.muscles.has("Triceps")}
                            onClick={() => toggleMuscle("Triceps")}
                            d="M45,110 Q40,150 45,185 L65,185 Q65,150 55,110 Z M155,110 Q160,150 155,185 L135,185 Q135,150 145,110 Z"
                          />
                          <DetailedMusclePath
                            name="Glutes"
                            active={newWorkout.muscles.has("Glutes")}
                            onClick={() => toggleMuscle("Glutes")}
                            d="M65,210 Q100,195 135,210 L140,245 Q100,260 60,245 Z"
                          />
                          <DetailedMusclePath
                            name="Hamstrings"
                            active={newWorkout.muscles.has("Hamstrings")}
                            onClick={() => toggleMuscle("Hamstrings")}
                            d="M70,255 L95,255 L85,340 L60,340 Z M105,255 L130,255 L140,340 L115,340 Z"
                          />
                        </>
                      )}
                    </svg>
                  ...
                  <button
                    onClick={() =>
                      setNewWorkout((p) => ({
                        ...p,
                        view: p.view === "front" ? "back" : "front",
                      }))
                    }
                    className="absolute bottom-10 flex items-center gap-4 px-10 py-4 bg-zinc-950 border border-white/10 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-emerald-500 hover:text-black transition-all group"
                  >
                ...
                <div className="flex flex-wrap justify-center gap-3">
                  {[
                    ...
                    <button
                      ...
                      className={`px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all ${newWorkout.muscles.has(m as MuscleGroup) ? "bg-emerald-500 border-emerald-400 text-black shadow-xl shadow-emerald-500/20 scale-105" : "bg-zinc-900 border-white/5 text-zinc-600 hover:text-zinc-300"}`}
                    >
                      {m}
                    </button>
                  ))}
                ...
                <button
                  disabled={newWorkout.muscles.size === 0}
                  onClick={() => setWorkoutStep("exercises")}
                  className="w-full py-10 bg-emerald-500 text-black rounded-[3.5rem] font-black uppercase italic tracking-widest flex items-center justify-center gap-6 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-10 shadow-3xl"
                >
            ...
            <div
              className={`flex-[1.5] p-12 bg-zinc-950 flex flex-col overflow-y-auto custom-scrollbar transition-all duration-700 ${workoutStep === "muscles" ? "opacity-30 pointer-events-none grayscale" : "opacity-100"}`}
            >
              <div className="max-w-4xl mx-auto w-full space-y-12 pb-24">
                <header className="space-y-6">
                  <h3 className="text-xs font-black uppercase tracking-[0.4em] text-zinc-500">
                    Phase II: movement stream definition
                  </h3>
                  <input
                    type="text"
                    placeholder="SESSION TITLE (E.G. PUSH PROTOCOL)..."
                    value={newWorkout.title}
                    onChange={(e) =>
                      setNewWorkout((p) => ({ ...p, title: e.target.value }))
                    }
                    className="w-full bg-transparent border-b-2 border-white/10 py-8 text-6xl font-black italic uppercase tracking-tight outline-none focus:border-emerald-500 transition-colors placeholder:text-zinc-900"
                  />
                </header>
                <div className="space-y-12">
                  <div className="flex items-center justify-between">
                    <h3 className="text-xs font-black uppercase tracking-[0.4em] text-zinc-500">
                      KINETIC CHAIN ENTRIES
                    </h3>
                    <button
                      onClick={addExercise}
                      className="flex items-center gap-4 bg-emerald-500/10 text-emerald-500 px-8 py-4 rounded-[2rem] border border-emerald-500/20 text-[11px] font-black uppercase tracking-widest hover:bg-emerald-500 hover:text-black transition-all shadow-xl"
                    >
                  ...
                  <div className="space-y-12">
                    {newWorkout.exercises.map((ex, exIdx) => (
                      <div
                        key={ex.id}
                        className="bg-zinc-900/40 p-12 rounded-[4.5rem] border border-white/5 space-y-10 group hover:bg-zinc-900/60 transition-all shadow-2xl"
                      >
                        <div className="flex items-center justify-between gap-8">
                          <input
                            ...
                            value={ex.name}
                            onChange={(e) => {
                              const n = [...newWorkout.exercises];
                              n[exIdx].name = e.target.value;
                              setNewWorkout((p) => ({ ...p, exercises: n }));
                            }}
                            className="bg-transparent font-black italic text-4xl uppercase outline-none flex-1 placeholder:text-zinc-800"
                          />
                          <button
                            onClick={() =>
                              setNewWorkout((p) => ({
                            ...
                            className="text-zinc-800 hover:text-red-500 transition-colors p-4"
                          >
                        ...
                        <div className="space-y-8">
                          ...
                          <div className="space-y-5">
                            {ex.sets.map((set, sIdx) => (
                              <div
                                key={set.id}
                                className="grid grid-cols-12 gap-8 items-center bg-black/40 p-6 rounded-[2.5rem] border border-white/5 group/set"
                              >
                                <div className="col-span-2 text-center font-black italic text-zinc-600 text-3xl">
                                  {sIdx + 1}
                                </div>
                                <div className="col-span-4 flex items-center gap-3">
                                  <button
                                    ...
                                    className="p-4 bg-zinc-900 rounded-2xl hover:text-emerald-500 transition-all"
                                  >
                                  ...
                                  <input
                                    ...
                                    className="w-full bg-zinc-900 border border-white/5 rounded-2xl py-5 text-center font-black italic tabular-nums text-emerald-400 outline-none"
                                  />
                                  <button
                                    onClick={() =>
                                      updateSet(
                                ...
                                <div className="col-span-4 flex items-center gap-3">
                                  ...
                                  <input
                                    ...
                                    className="w-full bg-zinc-900 border border-white/5 rounded-2xl py-5 text-center font-black italic tabular-nums outline-none"
                                  />
                                  <button
                                    onClick={() =>
                                      updateSet(
                                ...
                                <div className="col-span-2 text-right">
                                  <button
                                    onClick={() => {
                                      const n = [...newWorkout.exercises];
                                      n[exIdx].sets.splice(sIdx, 1);
                                      setNewWorkout((p) => ({
                                        ...p,
                                        exercises: n,
                                      }));
                                    }}
                                    className="text-zinc-800 hover:text-red-500 p-4 opacity-0 group-hover/set:opacity-100 transition-all"
                                  >
                                    <X className="w-6 h-6" />
                                  </button>
                                </div>
                          ...
                          <button
                            ...
                            className="w-full py-8 border-2 border-dashed border-white/5 rounded-[3rem] text-[12px] font-black uppercase tracking-widest text-zinc-700 hover:text-white hover:border-white/20 transition-all"
                          >
                            + APPEND NEXT SERIES
                          </button>
                        </div>
                ...
                <div className="flex gap-6 pt-16">
                  <button
                    onClick={resetNewWorkout}
                    className="flex-1 py-10 bg-zinc-900 text-zinc-700 rounded-[3.5rem] font-black uppercase italic tracking-widest hover:text-white transition-all"
                  >
                    DISCARD SESSION
                  </button>
                  <button
                    ...
                    className="flex-[2] py-10 bg-emerald-500 text-black rounded-[3.5rem] font-black uppercase italic tracking-widest shadow-3xl active:scale-95 transition-all disabled:opacity-10 flex items-center justify-center gap-6"
                  >
...
const MacroProgressCard: React.FC<{
  label: string;
  current: number;
  target: number;
  color: string;
  ...
  return (
    <div className="bg-gradient-to-br from-white/10 via-white/5 to-transparent border border-white/10 p-10 rounded-[3.5rem] backdrop-blur-xl group hover:from-emerald-500/20 hover:via-emerald-500/10 hover:border-emerald-500/30 transition-all shadow-xl hover:shadow-2xl hover:shadow-emerald-500/20">
      <div className="flex justify-between items-end mb-8">
        <div>
          <p className="text-[11px] font-black uppercase text-zinc-500 tracking-widest">
            {label}
          </p>
          <p
            className={`text-5xl font-black italic tracking-tighter tabular-nums leading-none mt-2 ${color}`}
          >
            {current}
          </p>
        </div>
      ...
      <div className="h-2 bg-white/5 rounded-full overflow-hidden border border-white/5">
        <div
          className={`h-full transition-all duration-1000 bg-gradient-to-r from-emerald-500 to-emerald-400 shadow-lg shadow-emerald-500/50`}
          style={{ width: `${percent}%` }}
        />
      </div>
    </div>
...
const NavButton: React.FC<{
  ...
  <button
    onClick={onClick}
    className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all backdrop-blur-md ${active ? "bg-gradient-to-br from-emerald-500 to-emerald-600 text-black shadow-2xl shadow-emerald-500/50 scale-110" : "text-zinc-500 bg-white/5 hover:text-emerald-400 hover:bg-white/10 hover:shadow-lg"} ${className}`}
  >
    {React.cloneElement(icon, { className: "w-6 h-6" })}
  </button>
);

const SummaryCard: React.FC<{ label: string; value: string; icon: any }> = ({
  label,
  value,
  icon,
}) => (
  <div className="bg-gradient-to-br from-white/10 via-white/5 to-transparent border border-white/10 p-12 rounded-[3.5rem] hover:from-emerald-500/20 hover:via-emerald-500/10 hover:to-transparent hover:border-emerald-500/30 transition-all group backdrop-blur-xl shadow-xl hover:shadow-2xl hover:shadow-emerald-500/20">
    <div className="p-5 bg-gradient-to-br from-emerald-500/20 to-emerald-500/5 rounded-2xl w-fit mb-10 group-hover:scale-110 group-hover:from-emerald-500/40 transition-transform shadow-lg">
      {icon}
    </div>
    <p className="text-5xl font-black italic tracking-tighter tabular-nums leading-none">
      {value}
    </p>
    <p className="text-[11px] font-black uppercase text-zinc-500 mt-4 tracking-widest">
      {label}
    </p>
  </div>
);

const StatMiniRow: React.FC<{ label: string; value: string }> = ({
  label,
  value,
}) => (
  <div className="flex items-center justify-between py-6 border-b border-emerald-500/20 last:border-0 hover:bg-emerald-500/5 transition-all px-4 rounded-lg">
    ...
    <span className="text-3xl font-black italic text-emerald-400 font-mono tracking-tighter tabular-nums drop-shadow-lg">
      {value}
    </span>
  </div>
);

const DetailedMusclePath: React.FC<{
  name: string;
  d: string;
  active: boolean;
  onClick: () => void;
}> = ({ name, d, active, onClick }) => (
  <path
    d={d}
    onClick={onClick}
    filter={active ? "url(#logNeon)" : ""}
    className={`cursor-pointer transition-all duration-700 outline-none drop-shadow-lg ${active ? "fill-emerald-500 drop-shadow-[0_0_20px_rgba(16,185,129,0.8)]" : "fill-zinc-700/40 hover:fill-zinc-600/60"}`}
  />
);
...
const StepInstruction: React.FC<{
  ...
  <div className="flex flex-col items-center gap-4 text-center group">
    <div className="w-14 h-14 rounded-full bg-gradient-to-br from-emerald-500/30 to-emerald-500/10 flex items-center justify-center text-emerald-400 font-black text-xl group-hover:scale-110 group-hover:from-emerald-500/50 group-hover:to-emerald-500/20 transition-all border border-emerald-500/40 shadow-lg">
      {num}
    </div>
    <div>
